...
export const contractors = pgTable(
  "contractors",
  {
    ...
    stripeRequirementsCurrentlyDue: jsonb("stripe_requirements_currently_due").$type<string[]>().notNull().default([]),
    stripeDisabledReason: text("stripe_disabled_reason"),
    stripeLastSyncedAt: timestamp("stripe_last_synced_at", { withTimezone: true, mode: "string" }),
    stripeConnectStatus: text("stripe_connect_status").notNull().default("not_connected"),
    /** Operator-set override for whether THIS profile's SnapLink/Southline plan has been paid — see lib/professional-intake-payment/. Never touched by Stripe Connect (that's the contractor collecting money from their own customers, a separate concern). Null = no manual override; a real subscription/invoice source takes precedence when one exists. */
    manualPaymentStatus: text("manual_payment_status"),
    manualPaymentNote: text("manual_payment_note"),
    manualPaymentSetAt: timestamp("manual_payment_set_at", { withTimezone: true, mode: "string" }),
    manualPaymentSetBy: text("manual_payment_set_by"),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
      .notNull()
      .defaultNow(),
  },
  (t) => [uniqueIndex("contractors_username_idx").on(t.username), index("contractors_stripe_account_idx").on(t.stripeAccountId)]
);
...
export const professionalIntakeSessions = pgTable(
  "professional_intake_sessions",
  {
    ...
    /** Content-approval step (Phase 5 of the payment-gating slice) — distinct from `appliedAt`: content can be approved independent of when fields were applied to the profile. */
    contentApprovedAt: timestamp("content_approved_at", { withTimezone: true, mode: "string" }),
    contentApprovedBy: text("content_approved_by"),
  },
...
// Phase 11 OAuth consent/connected-app administration and marketplace billing foundation.
// No pricing, MLS vendor, or payment-processor contract is hardcoded: realEstateBillingPlans
// amounts are operator-entered, and the payment-processor client is a pluggable abstraction
// (lib/real-estate/marketplace/billing.ts) with a safe preview fallback, matching the existing
// email/SMS provider pattern in lib/real-estate/integrations.
export const realEstateOauthConsents=pgTable("real_estate_oauth_consents",{id:text("id").primaryKey(),tenantId:text("tenant_id").notNull(),membershipId:text("membership_id").notNull().references(()=>realEstateMemberships.id),clientId:text("client_id").notNull(),scopes:jsonb("scopes").$type<string[]>().notNull().default([]),grantedAt:timestamp("granted_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),revokedAt:timestamp("revoked_at",{withTimezone:true,mode:"string"}),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true,mode:"string"}).notNull().defaultNow()},t=>[unique("re_oauth_consent_unique").on(t.tenantId,t.membershipId,t.clientId),index("re_oauth_consent_member_idx").on(t.tenantId,t.membershipId)]);
export const realEstateBillingPlans=pgTable("real_estate_billing_plans",{id:text("id").primaryKey(),providerKey:text("provider_key"),name:text("name").notNull(),billingPeriod:text("billing_period").notNull().default("monthly"),amountCents:integer("amount_cents").notNull().default(0),currency:text("currency").notNull().default("usd"),meteredUnit:text("metered_unit"),isActive:boolean("is_active").notNull().default(true),createdByMembershipId:text("created_by_membership_id").references(()=>realEstateMemberships.id),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true,mode:"string"}).notNull().defaultNow()},t=>[uniqueIndex("re_billing_plan_name_unique").on(t.name)]);
export const realEstateBillingSubscriptions=pgTable("real_estate_billing_subscriptions",{id:text("id").primaryKey(),tenantId:text("tenant_id").notNull(),organizationId:text("organization_id").notNull(),planId:text("plan_id").notNull().references(()=>realEstateBillingPlans.id),installationId:text("installation_id").references(()=>realEstateIntegrationInstallations.id),status:text("status").notNull().default("trialing"),currentPeriodStart:timestamp("current_period_start",{withTimezone:true,mode:"string"}).notNull().defaultNow(),currentPeriodEnd:timestamp("current_period_end",{withTimezone:true,mode:"string"}),cancelAt:timestamp("cancel_at",{withTimezone:true,mode:"string"}),externalCustomerReference:text("external_customer_reference"),createdByMembershipId:text("created_by_membership_id").references(()=>realEstateMemberships.id),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),canceledAt:timestamp("canceled_at",{withTimezone:true,mode:"string"})},t=>[index("re_billing_sub_tenant_idx").on(t.tenantId,t.organizationId),index("re_billing_sub_status_idx").on(t.tenantId,t.status)]);
export const realEstateBillingUsageRecords=pgTable("real_estate_billing_usage_records",{id:text("id").primaryKey(),tenantId:text("tenant_id").notNull(),subscriptionId:text("subscription_id").notNull().references(()=>realEstateBillingSubscriptions.id,{onDelete:"cascade"}),meteredUnit:text("metered_unit").notNull(),quantity:integer("quantity").notNull().default(1),occurredAt:timestamp("occurred_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow()},t=>[index("re_billing_usage_sub_idx").on(t.tenantId,t.subscriptionId,t.occurredAt)]);
export const realEstateBillingInvoices=pgTable("real_estate_billing_invoices",{id:text("id").primaryKey(),tenantId:text("tenant_id").notNull(),subscriptionId:text("subscription_id").notNull().references(()=>realEstateBillingSubscriptions.id),externalId:text("external_id").notNull(),periodStart:timestamp("period_start",{withTimezone:true,mode:"string"}).notNull(),periodEnd:timestamp("period_end",{withTimezone:true,mode:"string"}).notNull(),amountDueCents:integer("amount_due_cents").notNull().default(0),currency:text("currency").notNull().default("usd"),status:text("status").notNull().default("draft"),lineItems:jsonb("line_items").$type<Array<Record<string,unknown>>>().notNull().default([]),issuedAt:timestamp("issued_at",{withTimezone:true,mode:"string"}),dueAt:timestamp("due_at",{withTimezone:true,mode:"string"}),paidAt:timestamp("paid_at",{withTimezone:true,mode:"string"}),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow()},t=>[unique("re_billing_invoice_external_unique").on(t.tenantId,t.externalId),index("re_billing_invoice_sub_idx").on(t.tenantId,t.subscriptionId)]);

// ---------------------------------------------------------------------------
// Snaplink Profile — self-service real estate agent profiles (Southline Living).
// Deliberately NOT prefixed real_estate_*: that prefix is the multi-tenant brokerage
// SaaS (lib/real-estate/*). This is a separate consumer lead-gen product, mirroring
// how `contractors` works. Billing reuses lib/real-estate/marketplace/billing.ts
// unmodified via a synthetic per-profile tenant/org scope (see lib/agent-profiles/billing.ts).
// ---------------------------------------------------------------------------
// Identity/publishing columns added for the Agent Management slice (operator-created
