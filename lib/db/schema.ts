...
// ---------------------------------------------------------------------------
export const agentProfiles=pgTable("agent_profiles",{id:text("id").primaryKey(),slug:text("slug").notNull(),status:text("status").notNull().default("pending"),pin:text("pin"),name:text("name").notNull(),brokerageName:text("brokerage_name").notNull().default(""),licenseNumber:text("license_number").notNull().default(""),phone:text("phone").notNull().default(""),email:text("email").notNull(),serviceArea:text("service_area").notNull().default(""),bio:text("bio").notNull().default(""),tagline:text("tagline"),photoUrl:text("photo_url"),languages:jsonb("languages").$type<string[]>().notNull().default([]),specialties:jsonb("specialties").$type<string[]>().notNull().default([]),serviceAreas:jsonb("service_areas").$type<string[]>().notNull().default([]),yearsExperience:integer("years_experience"),tier:text("tier"),billingTenantId:text("billing_tenant_id"),billingOrganizationId:text("billing_organization_id"),billingSubscriptionId:text("billing_subscription_id"),createdAt:timestamp("created_at",{withTimezone:true,mode:"string"}).notNull().defaultNow(),updatedAt:timestamp("updated_at",{withTimezone:true,mode:"string"}).notNull().defaultNow()},t=>[uniqueIndex("agent_profiles_slug_idx").on(t.slug),index("agent_profiles_status_idx").on(t.status)]);
