...
// One static asset per locale — never a single shared image reused across
// languages. Regenerated by scripts/gen-og-image.mjs from one resolved
// per-locale copy object, so headline/body/footer text can never mix locales.
const DEFAULT_OG_IMAGE_BY_LANG: Record<Lang, string> = {
  en: "/og-image.png",
  es: "/og-image-es.png",
};
// Alt text for the OG image itself must resolve from the same locale as
// everything else on the card — previously hardcoded to the (English) site
// name for every locale.
const OG_IMAGE_ALT_BY_LANG: Record<Lang, string> = {
  en: "Ideas for every home",
  es: "Ideas para cada hogar",
};
...
export function buildSeoMetadata({
  ...
  const ogImage = page?.openGraphImageUrl ?? seo?.defaultOpenGraphImageUrl ?? DEFAULT_OG_IMAGE_BY_LANG[lang];
  const ogImageAlt = page?.openGraphImageUrl || seo?.defaultOpenGraphImageUrl ? siteName : OG_IMAGE_ALT_BY_LANG[lang];
