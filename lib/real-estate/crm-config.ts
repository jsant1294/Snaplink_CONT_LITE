...
export interface CrmField {
  key: string;
  label: string;
  type?: "text" | "email" | "number" | "textarea" | "datetime-local" | "checkbox" | "select" | "image" | "relationship";
  relationship?: "agents" | "brokerages" | "properties" | "buyers";
...
export const CRM_CONFIG: Record<CrmResource, { title: string; singular: string; fields: CrmField[]; columns: string[] }> = {
  ...
  agents: { title: "Agents", singular: "Agent", columns: ["firstName", "lastName", "email", "licenseNumber", "isActive"], fields: [
    { key: "organizationId", label: "Organization ID", required: true }, { key: "brokerageId", label: "Brokerage", type: "relationship", relationship: "brokerages", required: true },
  ...
  buyers: { title: "Buyer CRM", singular: "Buyer", columns: ["name", "email", "budgetMinCents", "budgetMaxCents", "pipelineStage"], fields: [
    ...
    { key: "financingStatus", label: "Financing status" }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents" },
  ...
  sellers: { title: "Seller CRM", singular: "Seller", columns: ["ownerName", "propertyAddress", "timeline", "pipelineStage"], fields: [
    ...
    { key: "repairs", label: "Repairs needed", type: "textarea" }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents" },
    { key: "pipelineStage", label: "Pipeline stage", type: "select", options: stages }, { key: "notes", label: "Notes", type: "textarea" },
  ]},
  leads: { title: "Lead Pipeline", singular: "Lead", columns: ["name", "leadType", "stage", "source", "assignedAgentId"], fields: [
    ...
    { key: "source", label: "Source" }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents" }, { key: "notes", label: "Notes", type: "textarea" },
  ]},
  showings: { title: "Showing Requests", singular: "Showing", columns: ["propertyId", "buyerId", "requestedAt", "status", "isApproved"], fields: [
    { key: "propertyId", label: "Property", type: "relationship", relationship: "properties", required: true }, { key: "buyerId", label: "Buyer", type: "relationship", relationship: "buyers" }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents", required: true },
  ...
  "open-houses": { title: "Open Houses", singular: "Open House", columns: ["propertyId", "startsAt", "endsAt", "attendeeCount", "isPublished"], fields: [
    { key: "propertyId", label: "Property", type: "relationship", relationship: "properties", required: true }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents", required: true },
  ...
  tasks: { title: "Assigned Tasks", singular: "Task", columns: ["title", "dueAt", "status", "assignedAgentId"], fields: [
    { key: "title", label: "Task", required: true }, { key: "assignedAgentId", label: "Assigned agent", type: "relationship", relationship: "agents" },
