...
async function execute(job: typeof realEstateJobs.$inferSelect) {
  ...
  else if (job.jobType === "marketplace.mls.sync") await runIncrementalSync(principal.scope.tenantId,String(p.installationId),p.syncType as MlsSyncType);
