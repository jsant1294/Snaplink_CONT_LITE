...
export async function selectors(scope: DataScope, type: string, query = "") {
  ...
  if (type === "sellers") return db().select({ value: realEstateSellers.id, label: realEstateSellers.ownerName }).from(realEstateSellers).where(and(eq(realEstateSellers.tenantId, scope.tenantId), isNull(realEstateSellers.deletedAt), query ? ilike(realEstateSellers.ownerName, q) : undefined, isAgentScope(scope) ? eq(realEstateSellers.assignedAgentId, scope.agentId) : undefined)).limit(30);
  if (type === "leads") return db().select({ value: realEstateLeads.id, label: realEstateLeads.name }).from(realEstateLeads).where(and(eq(realEstateLeads.tenantId, scope.tenantId), isNull(realEstateLeads.deletedAt), query ? ilike(realEstateLeads.name, q) : undefined, isAgentScope(scope) ? eq(realEstateLeads.assignedAgentId, scope.agentId) : undefined)).limit(30);
  if (type === "memberships") return db().select({ value: realEstateMemberships.id, label: realEstateMemberships.userEmail }).from(realEstateMemberships).where(and(eq(realEstateMemberships.tenantId, scope.tenantId), isNull(realEstateMemberships.deletedAt), eq(realEstateMemberships.isActive, true), query ? ilike(realEstateMemberships.userEmail, q) : undefined, isAgentScope(scope) ? eq(realEstateMemberships.id, scope.membershipId || "__none__") : undefined)).limit(30);
  return [];
}
