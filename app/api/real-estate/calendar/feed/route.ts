...
import { realEstateAppointments, realEstateOpenHouses, realEstateReminders, realEstateShowings } from "@/lib/db/schema";
...
export async function GET(req: NextRequest) {
  ...
  const [appointments, reminders, showings, houses] = await Promise.all([
    db().select().from(realEstateAppointments).where(and(eq(realEstateAppointments.tenantId, principal.tenantId), isNull(realEstateAppointments.deletedAt), agent ? eq(realEstateAppointments.assignedAgentId, agent) : undefined)),
  ...
  const events = [
    ...appointments.map(item => `BEGIN:VEVENT\nUID:${item.id}@snaplink\nDTSTART:${stamp(item.startsAt)}\nDTEND:${stamp(item.endsAt || new Date(new Date(item.startsAt).getTime() + 60 * 60000).toISOString())}\nSUMMARY:${safe(item.title)}\nEND:VEVENT`),
